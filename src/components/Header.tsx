...
import { Home, FileCheck2, Search, HelpCircle } from 'lucide-react'
  ...
  return (
    <header className="sticky top-0 z-50 bg-black/80 backdrop-blur-md border-b border-gray-800 px-6 h-14 flex items-center justify-between">
      <nav className="flex items-center gap-6 text-white text-sm font-medium">
        <Link href="/" className={`header-link flex items-center gap-1 ${pathname === '/' ? 'text-blue-400' : ''}`}>
          <Home size={16} /> Home
        </Link>
        ...
        <Link href="/faq" className={`header-link flex items-center gap-1 ${isFAQ ? 'text-blue-400' : ''}`}>
